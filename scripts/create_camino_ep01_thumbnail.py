...
SOURCE = ROOT / 'assets' / 'images' / 'camino_a2_ep01_la_idea' / 'scene_06_together.png'
THUMB_REPO = ROOT / 'thumbs' / 'camino-a2-ep01-la-idea-draft.jpg'
...
base = cover_crop(source, (W, H), focus=(0.50, 0.35))
...
# Focus keeps the three faces (upper-centre of the source art) in frame.
scene = cover_crop(source, (860, 720), focus=(0.62, 0.22)).convert('RGBA')
scene = ImageEnhance.Contrast(scene).enhance(1.02)
scene = ImageEnhance.Color(scene).enhance(1.02)
...
sd.rounded_rectangle((panel[0] + 8, panel[1] + 10, panel[2] + 8, panel[3] + 10), radius=38, fill=(0, 0, 0, 80))
shadow = shadow.filter(ImageFilter.GaussianBlur(8))
canvas = Image.alpha_composite(canvas, shadow)
d = ImageDraw.Draw(canvas)
d.rounded_rectangle(panel, radius=38, fill=(255, 248, 231, 218), outline=(255, 255, 255, 180), width=3)
...
# Badge.
badge_text = 'Español A2+ · Camino Ep.1'
...
# Main title: the episode key sentence.
x, y = 90, 198
for line in ['Quiero hacer', 'el Camino']:
    d.text((x, y), line, font=F_title, fill=(255, 255, 255), stroke_width=5, stroke_fill=(31, 35, 40))
    y += 76
