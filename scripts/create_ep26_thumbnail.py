...
SRC = ROOT / 'images' / 'ep26-estoy-en-la-estacion-source.png'
SOURCE_OUT = ROOT / 'images' / 'ep26-estoy-en-la-estacion-source.png'
THUMB_REPO = ROOT / 'thumbs' / 'a1-ep26-estoy-en-la-estacion.jpg'
THUMB_OUT = ROOT / 'output' / 'thumbs' / 'frases-a1-ep26-estoy-en-la-estacion.jpg'
QA_OUT = ROOT / 'output' / 'qa' / 'estoy-en-la-estacion' / 'thumbnail.jpg'
...
source16 = cover_crop(source_original, (W, H), focus=(0.57, 0.42))
...
base = cover_crop(source, (W, H), focus=(0.62, 0.45))
...
# Keep the station/Jin scene sharp on the right, faded into the text panel.
scene = cover_crop(source, (840, 720), focus=(0.86, 0.45)).convert('RGBA')
...
scene.putalpha(mask)
canvas.alpha_composite(scene, (440, 0))

# Dark left gradient for readability.
ov = Image.new('RGBA', (W, H), (0, 0, 0, 0))
od = ImageDraw.Draw(ov)
for x in range(0, 640):
    alpha = int(58 * (1 - x / 640))
...
center_text(d, badge, 'Español A1+ · Ep.26', font(25), fill=(255, 255, 255, 255))

# Main title.
d.text((90, 202), 'Estoy en', font=font(64), fill=(255, 255, 255, 255), stroke_width=5, stroke_fill=(18, 18, 18, 255))
d.text((92, 300), 'la estación', font=font(55), fill=(255, 255, 255, 255), stroke_width=5, stroke_fill=(18, 18, 18, 255))

# Korean pill.
pill_ko = (82, 438, 548, 508)
d.rounded_rectangle(pill_ko, radius=35, fill=(255, 255, 255, 250), outline=(16, 16, 16, 45), width=2)
center_text(d, pill_ko, '나 역에 있어요', font(34), fill=(24, 24, 24, 255))

# Teal learning-point pill.
pill_teal = (82, 540, 474, 600)
d.rounded_rectangle(pill_teal, radius=30, fill=(20, 150, 148, 255))
center_text(d, pill_teal, '위치 말하기 = estar', font(26), fill=(255, 255, 255, 255))

# Small station accent.
for x, y, r, c in [
    (530, 136, 8, (255, 198, 88, 255)),
    (552, 158, 5, (255, 220, 135, 255)),
    (528, 596, 7, (105, 195, 215, 255)),
    (564, 596, 4, (70, 120, 190, 255)),
]:
