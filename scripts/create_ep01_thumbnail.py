...
SOURCE_OUT = ROOT / 'assets' / 'story' / 'lucia_scene_07_churros.png'
THUMB_REPO = ROOT / 'thumbs' / 'historia-a1-ep01-un-dia-de-lucia.jpg'
THUMB_OUT = ROOT / 'output' / 'thumbs' / 'historia-un-dia-de-lucia.jpg'
QA_OUT = ROOT / 'output' / 'qa' / 'historia-un-dia-de-lucia' / 'thumbnail.jpg'
...
# To avoid overlapping characters with the left cream panel:
# 1. We shift the scene start X position from 420 to 580 (pushing it right).
# 2. We reduce the crop width from 860 to 700.
# 3. We focus heavily on the right side of the source image (focus_x = 0.72) to capture Lucía clearly.
scene_w = 700
scene = cover_crop(source, (scene_w, 720), focus=(0.72, 0.50)).convert('RGBA')
scene = ImageEnhance.Contrast(scene).enhance(1.05)
scene = ImageEnhance.Color(scene).enhance(1.04)

# Create a narrower gradient mask (120px transition instead of 190px)
mask = Image.new('L', scene.size, 255)
md = ImageDraw.Draw(mask)
fade_w = 120
for i in range(fade_w):
    md.line([(i, 0), (i, scene.height)], fill=int(255 * (i / fade_w)))
scene.putalpha(mask)
canvas.alpha_composite(scene, (W - scene_w, 0)) # Position on the right edge (1280 - 700 = 580)

# Dark left gradient behind the cream panel.
ov = Image.new('RGBA', (W, H), (0, 0, 0, 0))
od = ImageDraw.Draw(ov)
for x in range(0, 680):
    alpha = int(55 * (1 - x / 680))
...
# Badge (Español A1 · Historia Ep.1) - Teal background (44, 150, 142) to distinguish from Card series
# Expanded badge width to fit the longer text
badge = (82, 116, 420, 168)
d.rounded_rectangle(badge, radius=25, fill=(44, 150, 142, 255))
center_text(d, badge, 'Español A1 · Historia Ep.1', font(22), fill=(255, 255, 255, 255))

# Main title: Un día de Lucía
d.text((88, 208), 'Un día de', font=font(72), fill=(255, 255, 255, 255), stroke_width=6, stroke_fill=(18, 18, 18, 255))
d.text((90, 310), 'Lucía', font=font(74), fill=(255, 255, 255, 255), stroke_width=6, stroke_fill=(18, 18, 18, 255))

# Korean meaning pill: 하루 일과 따라가기
pill_ko = (82, 448, 548, 518)
d.rounded_rectangle(pill_ko, radius=35, fill=(255, 255, 255, 250), outline=(16, 16, 16, 45), width=2)
center_text(d, pill_ko, '하루 일과 따라가기', font(32), fill=(24, 24, 24, 255))

# Teal learning-point pill: Primero · Luego · Al final
pill_teal = (82, 548, 512, 608)
d.rounded_rectangle(pill_teal, radius=30, fill=(20, 150, 148, 255))
center_text(d, pill_teal, 'Primero · Luego · Al final', font(24), fill=(255, 255, 255, 255))
