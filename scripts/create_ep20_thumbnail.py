...
SRC = Path('/opt/data/cache/images/openai_codex_gpt-image-2-medium_20260521_201108_c08e846e.png')
SOURCE_OUT = ROOT / 'images' / 'ep20-porque-tengo-sueno-source.png'
THUMB_REPO = ROOT / 'thumbs' / 'a1-ep20-porque-tengo-sueno.jpg'
THUMB_OUT = ROOT / 'output' / 'thumbs' / 'frases-a1-ep20-porque-tengo-sueno.jpg'
QA_OUT = ROOT / 'output' / 'qa' / 'porque-tengo-sueno' / 'thumbnail.jpg'
...
# Persist 16:9 source image for Ep.20.
SOURCE_OUT.parent.mkdir(parents=True, exist_ok=True)
source_original = Image.open(SRC).convert('RGB')
# Keep Jin/Lucía/Diego on right while preserving uncropped heads.
source16 = cover_crop(source_original, (W, H), focus=(0.74, 0.48))
...
# Sharp café scene on the right, faded into the text panel.
scene = cover_crop(source, (790, 720), focus=(0.86, 0.50)).convert('RGBA')
...
for i in range(150):
    md.line([(i, 0), (i, scene.height)], fill=int(255 * (i / 150)))
scene.putalpha(mask)
canvas.alpha_composite(scene, (490, 0))

# Subtle dark gradient on left for contrast.
ov = Image.new('RGBA', (W, H), (0, 0, 0, 0))
od = ImageDraw.Draw(ov)
for x in range(0, 620):
    alpha = int(54 * (1 - x / 620))
...
# Badge.
badge = (82, 118, 345, 170)
d.rounded_rectangle(badge, radius=25, fill=(235, 111, 87, 255))
center_text(d, badge, 'Español A1+ · Ep.20', font(25), fill=(255, 255, 255, 255))

# Main title.
main_f = font(59)
lines = ['Porque', 'tengo', 'sueño']
y = 204
for line in lines:
...
# Korean pill.
pill_ko = (82, 438, 494, 505)
d.rounded_rectangle(pill_ko, radius=32, fill=(255, 255, 255, 250), outline=(16, 16, 16, 45), width=2)
center_text(d, pill_ko, '졸려서요', font(38), fill=(24, 24, 24, 255))

# Teal phrase pill.
pill_teal = (82, 540, 390, 600)
d.rounded_rectangle(pill_teal, radius=30, fill=(20, 150, 148, 255))
center_text(d, pill_teal, '¿Por qué?', font(34), fill=(255, 255, 255, 255))

# Footer learning point.
d.text((407, 555), 'porque + 이유', font=font(23), fill=(78, 68, 58, 235))

# Small sleepy/café accent dots.
for x, y, r, c in [
    (526, 132, 8, (255, 198, 88, 255)),
    (540, 154, 5, (255, 220, 135, 255)),
    (514, 596, 7, (105, 195, 215, 255)),
]:
